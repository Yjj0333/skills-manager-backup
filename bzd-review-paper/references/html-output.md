...
- Preserve the exact visible section order required by `SKILL.md`. For CUMCM include the competition-context section between eligibility/format review and judge comments; omit it for other contests.
...
- Show the paper-quality score prominently in the circular display. For CUMCM also show provincial and national competition scores as separate cards; the percentile bar follows the competition score used for that rank estimate.
...
- For CUMCM replace `{{COMPETITION_METRIC_CARDS}}` with two metric cards labeled `省奖竞争修正分` and `国奖竞争修正分`, and replace `{{COMPETITION_CONTEXT_SECTION}}` with section number 04 containing the complete context audit.
- For non-CUMCM replace both competition placeholders with an empty string, and renumber the following section labels if desired; no empty context card may remain.
- The context section must contain a plain statement that the paper-quality score is independent of school, region, advisor and division. Show the exact formula deltas and any 6.81%/74.9/provincial-first ceiling.
...
2. Confirm the raw score, multiplier, paper-quality score, competition deltas, competition scores and percentile values match the report.
